...
getAgencyData= function(id, job, description, date, taxable, ncharacters, languages, jobdate, urgency, agency) {
	var url = "getAgencyData.php?user="+agency;
...
function generate_agency_invoice(id) {
	...
	req.onreadystatechange = function(){
		if (req.status == 200&req.readyState==4){
			...
			getAgencyData(data.id, data.Job, data.Description, data.Date, data.Price, data.ncharacters, data.languages, data.jobDate, data.urgency, data.agency)	
		}
	}
