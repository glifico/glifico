...
if($azione=="getUserProfilePicture") {
	...
	while($row = @mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$path = "/uploadedFiles/".$uid."/".$row[0]["IMAGE"];
		}
		else {
		//caso immagine non presente
		$path = "/images/profile_picture.jpg";			
		}
		echo "dfd".$path."sdf".$row['IMAGE'];
		$b64image = base64_encode(file_get_contents_curl($path));
...
//campi agenzia
		...
		if(!empty($pictureUrl) && $social == 'LINKEDIN') {
			...
			//VEDI UPLOAD.PHP
			$pictureUrl = "http://graph.facebook.com/" + $password + "/picture?type=large";
